...
$argv = $_SERVER['argv'];
if($argc <= 1) {
   echo "Usage: import_figure_data.php filename type highlight \n";
   echo "   ie. import_figure_data.php Figurepricing/Infinity/pricing.csv infinity loose\n"; 
   exit;
}
...
if($argv[2]) {$makeconsole = $argv[2];}
   ...
   echo "Please provide type name(i.e. skylander)\n";
   exit;
}
if($argv[3]) {
   if ($argv[3] == 'loose'){ $highlight = '0';}
   elseif ($argv[3] == 'new'){ $highlight = '1';}
   else{
      echo "highlighting must be either loose or new\n";
      exit;
   }
}
else {
   echo "Please state: loose or new\n";
   exit;
}
// --------------------------------------------------------------------
      ...
      if ($num != 7) {
         ...
         if ($dbrow = mysqli_fetch_assoc($result)) {
            ...
            $sql = "UPDATE gameprices SET `PriceLoose`='$updateloose',
                    `PriceCIB`='$updatecib', `PriceNew`='$updatenew',
